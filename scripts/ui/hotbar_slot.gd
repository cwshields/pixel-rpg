...
## Shows a framed slot, the item icon + stack count, and the hotkey digit
## in the corner; swaps its frame texture when selected.

const _BG_COLOR := Color(0.12, 0.11, 0.16, 0.9)
...
func set_selected(selected: bool) -> void:
	_frame.texture = _tex_selected if selected and _tex_selected else _tex_normal
